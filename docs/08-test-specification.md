...
| レイヤー | ツール | 対象 |
|----------|--------|------|
| ユニット | Vitest + @testing-library/react | `lib/validation.ts`、`lib/videos.ts`（並び順の組み立て）、`lib/request.ts`（JSON ボディ解析）、`lib/supabase/client.ts`（PKCE 設定）、各フック、主要コンポーネント（atoms/molecules/organisms）、Route Handler の認可単体（`auth/admin`・`openapi.json`） |
...
| 14 | キーボードのみで詳細遷移（カードの button へフォーカス → Enter） | アクセシビリティ |
| 15 | 一覧操作のアクセシブル名（検索 / 並び替え / ページ送り / `aria-current`） | アクセシビリティ |

> 備考: 仕様上は 15 ケースだが、Playwright 実装ではケース 11 の「次へ」検証をケース 11 内に含めているため、`public.spec.ts` の `test()` は 14 件。
