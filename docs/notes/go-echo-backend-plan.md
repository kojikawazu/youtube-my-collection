...
- [背景](#背景)
- [現在の Supabase テーブル一覧](#現在の-supabase-テーブル一覧)
- [現在のアーキテクチャ](#現在のアーキテクチャ)
- [移行後のアーキテクチャ](#移行後のアーキテクチャ)
  - [設計方針](#設計方針)
- [技術スタック（バックエンド）](#技術スタックバックエンド)
- [認証・認可設計](#認証認可設計)
  - [トークン検証方式](#トークン検証方式)
  - [認証フロー（変更なし）](#認証フロー変更なし)
  - [トークンリフレッシュ](#トークンリフレッシュ)
- [CORS 設計](#cors-設計)
  - [Echo CORS ミドルウェア設定](#echo-cors-ミドルウェア設定)
  - [注意点](#注意点)
- [移行対象 API エンドポイント](#移行対象-api-エンドポイント)
  - [VideoEntry ドメイン（本プロジェクト）](#videoentry-ドメイン本プロジェクト)
  - [レスポンス互換要件](#レスポンス互換要件)
- [フロント側の影響](#フロント側の影響)
  - [影響度: 低](#影響度-低)
  - [修正ファイル（3ファイル・6箇所）](#修正ファイル3ファイル6箇所)
  - [環境変数の追加](#環境変数の追加)
  - [変更不要のファイル](#変更不要のファイル)
  - [削除対象ファイル（移行完了後）](#削除対象ファイル移行完了後)
  - [削除対象パッケージ（package.json）](#削除対象パッケージpackagejson)
  - [E2E テスト](#e2e-テスト)
- [Go バックエンド 環境変数](#go-バックエンド-環境変数)
- [バリデーション](#バリデーション)
- [リスク評価](#リスク評価)
