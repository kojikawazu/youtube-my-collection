...
> **Zod 由来の型（`z.infer`）**: 型の実体はスキーマ側にあるため、**コメントもスキーマ（`schemas/`）のフィールドに書く**。導出先の `z.infer` エイリアスには型本体の説明のみ付ける（両方に書くと二重管理になる）。

### 書き方の規律（TypeScript strict 前提 / TSDoc スタイル）

- **型は書かない**。`@param {string}` のような型ブレースは付けない（型は TS シグネチャが唯一の真実。二重管理・型ずれの原因）。**例外は `@throws` のみ**（後述。TypeScript に throws 句が無く、シグネチャに現れない情報のため再掲に当たらない）。
...
- **意図的に例外を投げる場合は `@throws {ErrorType} 発生条件` を書く**。呼び出し側は throw の有無も種類もシグネチャから知れないため、コメントが唯一の手掛かりになる。
  - **`@throws` は「型を書かない」の例外**であり、**型を明記する**。TypeScript には throws 句が無く、**投げる例外の型はシグネチャのどこにも現れない**ため、これは再掲ではなく情報の追加になる。型が無いと呼び出し側は何を `catch` して分岐すべきか判断できない。
  - `jsdoc/no-types` は `@param` / `@returns` の型ブレースを検出するが、**`@throws` は対象外**なので設定変更なしで併用できる。
  - **対象は「関数外へ伝播する例外」だけ**。関数内の `try` / `catch` で捕捉して握り潰す・別の値へ変換する場合は書かない（呼び出し側に例外は届かないため）。
...
| ルール | 重大度 | 目的 |
|---|---|---|
| `jsdoc/no-types` | error | JSDoc への型の再掲を禁止（TS シグネチャが唯一の真実）。**`@param` / `@returns` が対象で `@throws` は対象外**（上記のとおり例外の型はシグネチャに現れないため） |
