...
| `useVideos` | 一覧取得・検索・ソート・ページネーション | `videos`, `totalCount`, `currentPage`, `sortOption`, `searchQuery`, `isLoading`, `loadError`, `totalPages`, `visiblePageNumbers` | `setCurrentPage()`, `setSortOption()`, `setSearchQuery()`, `refreshListPage()` |
...
```
useToast          ← 依存なし
useAuth           ← showToast を受け取る
useVideos         ← accessToken, showToast を受け取る
useVideoForm      ← accessToken, showToast, refreshListPage を受け取る
...
E2Eテストは管理者操作を対象外としているため（`docs/04.e2e-cases.md:17`）、各Phaseで以下を手動確認する。

- [ ] 許可メールでGoogleログイン → 管理者バッジ表示・FAB表示
- [ ] 非許可メールでGoogleログイン → 「権限がありません」トースト表示・ログアウト状態に戻る
- [ ] 管理者ログアウト → 管理者バッジ非表示・FAB非表示・リスト画面に戻る
- [ ] 動画追加（FABクリック → フォーム入力 → 保存モーダル確定） → 「追加しました。」トースト・リストに反映
- [ ] 動画編集（詳細 → 編集ボタン → フォーム変更 → 保存モーダル確定） → 「更新しました。」トースト・詳細に反映
- [ ] 動画削除（カード上の削除ボタン → 削除モーダル確定） → 「削除しました。」トースト・リストから消える
- [ ] 動画削除（詳細画面の削除ボタン → 削除モーダル確定） → リスト画面に戻る
- [ ] バリデーションエラー（タイトル空・URL空） → エラー表示・保存されない
- [ ] 保存/削除のAPI失敗時 → モーダルが閉じない・エラーアラート表示・再操作可能
- [ ] モーダル確定ボタン連打 → 二重送信されない（ボタンが disabled になる）
- [ ] モーダル閉じた後 → `body` のスクロール（`overflow`）が元に戻る
...
- **元の位置**: page.tsx 39-53行(state), 71-118行(loadVideos/refresh), 120-140行(effects)
- **責務**: 動画一覧の取得、検索・並び替え・ページネーション
- **依存**: なし（純粋なデータ取得）
- **公開API**:
  - `videos: VideoItem[]`
  - `totalCount: number`
...
- **依存**: `accessToken`, `showToast`, `refreshListPage` を引数で受け取る
- **公開API**:
  - `formData: Partial<VideoItem>`
  - `formErrors: ValidationErrors`
