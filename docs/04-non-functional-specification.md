...
| 一覧取得のレスポンス到着順が逆転 | **最新のリクエストだけが `videos` / `totalCount` / `isLoading` / `loadError` を更新する**。前回の通信は `AbortController` で中止し、加えて連番 ID で最新かを確認する（abort はネットワークを止めるだけで、応答受信済みの処理は止まらないため）。中止は正常な打ち切りとしてエラー表示しない |
