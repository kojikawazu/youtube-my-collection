...
| S-3 | 保存 API 失敗 → モーダルが閉じない・再操作可能 | admin session 注入, POST mock 500 | FAB → フォーム入力 → 保存モーダル確定 | `alert()` ダイアログが表示される（`page.on('dialog', d => d.accept())` で受け入れ）、モーダルが開いたまま、確認ボタンが再び有効になる | High |
